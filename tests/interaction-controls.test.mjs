...
test("Fraud Orders drawers close without unsaved draft state", () => {
  ...
  assert.match(reviewSource, /commandFor=\{BOTSHIELD_FRAUD_REVIEW_MODAL_ID\}/);
  assert.match(reviewSource, /command="--hide"/);
...
test("Settings destructive confirmation uses native modal command pattern", () => {
  ...
  assert.match(settingsSource, /RESET_BOTSHIELD_DATA_DESCRIPTION/);
