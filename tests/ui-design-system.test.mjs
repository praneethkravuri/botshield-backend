...
test("new app shell keeps a simplified Shopify-native app navigation", async () => {
  ...
  assert.equal([...source.matchAll(/label: "Value"/g)].length, 1);
...
test("destructive settings actions use App Bridge confirmation modal", async () => {
  ...
  const modalCommandSource = await readFile(
  ...
  assert.match(modalCommandSource, /trigger\.setAttribute\("command", command\)/);
