...
const BOT_DETECTION_REASON_CODES = [
...
const RATE_PROTECTION_REASON_CODES = [
  "RATE_PATTERN",
  "REPEAT_OFFENDER",
  "PATH_SCANNING",
];
...
function eventMatchesReasonCodes(event, reasonCodes) {
  const codes = getOverviewReasonCodes(event);
...
function buildModuleProtectionActivity(storefrontScans, reasonCodes, periodDays = 30, nowMs = 0) {
  ...
  const blocked = matchedEvents.filter(
    (event) => event.actionTaken === "blocked",
  ).length;
  const challenged = matchedEvents.filter(
    (event) => event.actionTaken === "challenged",
  ).length;
  return {
    detected: matchedEvents.length,
    blocked,
    challenged,
    ...
    <BotShieldNativePage heading="Analytics">
      <BotShieldPageShell className="botshield-analytics-content botshield-analytics-v2 botshield-analytics-premium">
        ...
        {model.syncing && !model.storefrontScans?.length ? (
          ...
          <>
        <header className="ba-page-intro">
          <div className="ba-page-intro-copy">
            <span>Threat intelligence</span>
            <p>Review storefront security activity, trends, and individual events for the selected period.</p>
          </div>
          <div className="ba-page-intro-actions">
        ...
        <section className="botshield-analytics-controls" aria-label="Analytics filters">
        ...
        <section className="botshield-analytics-kpis" aria-label="Analytical metrics">
          <AnalyticsKpi animateNumber label="Suspicious events" value={suspiciousEvents.length} detail="Storefront events with elevated threat signals" />
          ...
          <AnalyticsKpi animateNumber label="High-risk activity" value={highRiskCount} detail="Events classified as high risk" />
        </section>

        <div className="botshield-analytics-section-label">Activity over time</div>
        <AnalyticsPanel className="ba-primary-chart" title="Activity patterns" subtitle="See when suspicious storefront activity is most concentrated.">
        ...
        <AnalyticsPanel className="ba-event-explorer-panel" title="Event explorer" subtitle="Filter and review the storefront events behind these metrics.">
...
function AnalyticsKpi({ label, value, detail, compact = false, animateNumber = false }) {
  return (
    <div className={`botshield-analytics-kpi${compact ? " is-compact" : ""}`}>
      <span>{label}</span>
      <strong>
        {animateNumber && typeof value === "number" ? (
          <OverviewAnimatedNumber value={value} />
        ) : typeof value === "number" ? (
          formatHydrationStableNumber(value)
        ) : (
          value
        )}
      </strong>
...
function AnalyticsPanel({ title, subtitle, children, className = "" }) {
  return (
    <section className={`botshield-analytics-panel${className ? ` ${className}` : ""}`}>
...
const FRAUD_FILTER_EMPTY = {
  ...
  all: {
    ...
    description: "Shopify returned no recent orders for this store.",
  },
};
...
function getFraudQueueEmptyState({
  ...
  if (!connected) {
    return {
      ...
      description: "Connect order access to review Shopify order risk in BotShield.",
  ...
  if (hasOrders && (trimmedSearch || filterKey !== "all")) {
    return {
  ...
  if (filterKey === "needs-review" && hasOrders) {
    return {
      title: filterEmpty.title,
      description: filterEmpty.description,
      actionLabel: filterEmpty.actionLabel,
...
function fraudOrderRiskBadgeTone(riskTone) {
  if (riskTone === "high") return "critical";
...
function FraudOrderStatusStrip({ onSetup }) {
  return (
    <BotShieldBanner
      action={<BotShieldActionButton onClick={onSetup}>Review setup</BotShieldActionButton>}
      title="Order access required"
      tone="warning"
    >
      Connect order access so BotShield can read supported Shopify order-risk information for
      this store.
    </BotShieldBanner>
  );
}
...
const FRAUD_ORDER_TABLE_HEADINGS = [
  "Order",
  "Total",
  "Risk",
  "Recommendation",
...
function FraudOrderResourceTable({ loading, onReview, orders, riskLabel, riskTone }) {
...
function FraudOrdersPage({ model, actions }) {
  ...
  const renderAccessBanner = () => {
    ...
    return <FraudOrderStatusStrip onSetup={openSetup} />;
  };

  const renderErrorBanner = () => {
    if (!connected || !error) return null;

    if (errorCode === "protected_customer_data") {
      return (
        <BotShieldBanner
          action={
            <>
              <BotShieldActionButton onClick={refresh}>Retry</BotShieldActionButton>
              <BotShieldActionButton onClick={openSetup}>Review setup</BotShieldActionButton>
            </>
          }
          title="Order data access isn't available yet"
          tone="warning"
        >
          Shopify protected customer data approval is required before BotShield can load order risk
    ...
    return (
      <BotShieldBanner
        ...
        tone="critical"
      >
        Try again in a moment. If the issue continues, review your order access setup.
      </BotShieldBanner>
    );
  };
  ...
  const renderQueueBody = () => {
    ...
    if (connected && !error && filteredOrders.length) {
      return (
        <FraudOrderResourceTable
          loading={loading}
          onReview={openReview}
    ...
    const emptyState = getFraudQueueEmptyState({
      ...
      onViewAllOrders: () => setActiveFilter("all"),
      orders,
      search,
    });
...
const BOT_PROTECTION_MODULE = "bot";
const PROTECTION_POLICY_MODULE = "policy";

function isBotProtectionModal(modal) {
  return (
    modal?.module === BOT_PROTECTION_MODULE || modal?.title === "Bot Protection"
  );
}

function isProtectionPolicyModal(modal) {
  return (
    modal?.type === "profile" && modal?.module === PROTECTION_POLICY_MODULE
  );
}

function isEditableProtectionProfileModal(modal) {
  return (
    modal?.type === "profile" &&
    !isBotProtectionModal(modal) &&
    (modal?.module === PROTECTION_POLICY_MODULE || modal?.module === "rate")
  );
}

function getProtectionModalKey(modal) {
  if (!modal) return "closed";
  return `${modal.type}:${modal.module || "none"}:${modal.title || "protection"}`;
}
...
function ProtectionPage({ model, actions }) {
  ...
  const handleProtectionModalAfterHide = () => {
    ...
    if (transition === "open-policy") {
      ...
      setProtectionModal({
        ...
        module: PROTECTION_POLICY_MODULE,
        note:
  ...
  const pageStatus = storefrontConnected
    ? {
        label: model.protectionPaused ? "Paused" : "Connected",
        status: model.protectionPaused ? "paused" : "active",
      }
    : { label: "Needs setup", status: "setup_required" };

  const openProfileManager = (title, text, note, module = PROTECTION_POLICY_MODULE) => {
    if (module === BOT_PROTECTION_MODULE || title === "Bot Protection") {
  ...
  const createBotProtectionModalState = () => ({
    type: "status",
    title: "Bot Protection",
    text: "Detects automated browsers, suspicious user-agent patterns, and request-integrity gaps.",
    note: "Bot Protection evaluates automation signals on storefront requests. Enforcement follows your active protection policy.",
    status: moduleStatus,
    module: BOT_PROTECTION_MODULE,
  });
  ...
  const openBotProtectionModule = () => {
    ...
    drawerOpenerRef.current = document.activeElement;
    setSaveError("");
    setProtectionModal(createBotProtectionModalState());
  };
  ...
  const openRateProtectionModule = () =>
    openProfileManager(
      ...
      "Rate Protection adds behavioral signals to the shared risk score. Enable or disable individual signals below.",
      "rate",
    );
  ...
  const protectionRows = [
    ...
    },
    {
      ...
      configLabel: "Rate signals",
      configValue: `${
        [
          model.repeatedActivityEnabled,
          model.elevatedRateEnabled,
          model.burstTrafficEnabled,
          model.repeatOffenderEnabled,
          model.pathScanningEnabled,
  ...
  const moduleVerifiedActivity =
    protectionModal?.module === "rate"
      ? rateProtectionActivity
      : {
          detected: interventionCount,
          blocked: Number(model.incidentCounts?.blocked || 0),
          challenged: Number(model.incidentCounts?.challenged || 0),
  ...
  useEffect(() => {
    ...
    if (
      ...
      isBotProtectionModal(protectionModal)
    ) {
      setProtectionModal(createBotProtectionModalState());
    }
    return undefined;
  ...
  if (model) {
    return (
      <BotShieldNativePage heading="Protection">
        ...
        <BotShieldNativeModal
            key={getProtectionModalKey(protectionModal)}
          ...
          >
            ...
            {isEditableProtectionProfileModal(protectionModal) ? (
              <>
                ...
                {isProtectionPolicyModal(protectionModal) ? (
                  <>
                ...
                <section className="botshield-protection-drawer-section botshield-protection-drawer-section--compact">
                  ...
                  <div className="botshield-protection-drawer-metrics">
                    <div>
                      <strong>{moduleVerifiedActivity.detected}</strong>
                      <span>
                        {protectionModal.module === "rate"
                          ? "Rate signals"
                          : "Evaluated"}
                    ...
                    <div>
                      <strong>{moduleVerifiedActivity.blocked}</strong>
                      <span>Blocked</span>
                    </div>
                    <div>
                      <strong>{moduleVerifiedActivity.challenged}</strong>
                ...
                <BotShieldInlineHelp>
                  {isProtectionPolicyModal(protectionModal)
                    ? "This policy applies across Bot Protection, Rate Protection, Network / Proxy Protection, and Page Protection."
                    ...
                    <section className="botshield-protection-drawer-section">
                      ...
                      <div className="botshield-protection-signal-list">
                        ...
                        <div>
                          ...
                          <span>Rapid access across multiple storefront paths</span>
                        </div>
                      </div>
                    </section>
                    <section className="botshield-protection-drawer-section botshield-protection-drawer-section--compact">
                      ...
                      <div className="botshield-protection-drawer-metrics">
                        <div>
                          <strong>{botProtectionActivity.detected}</strong>
                          <span>Bot signals</span>
                        </div>
                        <div>
                          <strong>{botProtectionActivity.blocked}</strong>
                          <span>Blocked</span>
                        </div>
                        <div>
                          <strong>{botProtectionActivity.challenged}</strong>
                          <span>Challenged</span>
                        </div>
                      </div>
                    </section>
                    <BotShieldInlineHelp>
                      Bot Protection identifies automation signals. Enforcement
                      thresholds and responses are configured in Protection policy.
                    </BotShieldInlineHelp>
                  </>
                ) : (
                  <>
...
function SettingsHubFixedAction({ children }) {
...
function SettingsHubFixedValue({ children }) {
  return (
    <div className="botshield-settings-hub-fixed-value">
      <span className="botshield-settings-hub-value">{children}</span>
    </div>
  );
}
  ...
  const renderSection = () => {
    ...
    if (activeSection === "privacy") {
      return (
        ...
        >
          <SettingsHubRow
            control={<SettingsHubFixedValue>{recordedActivityLabel}</SettingsHubFixedValue>}
            description="Storefront security and protection activity recorded for analytics, alerts, investigation, and enforcement."
            icon="activity"
            title="Recorded activity"
            variant="info"
          />
          <SettingsHubRow
            control={<SettingsHubFixedValue>{simulationLabel}</SettingsHubFixedValue>}
            description="Simulation and test activity stays separate from live storefront metrics and reports."
            icon="diagnostic"
